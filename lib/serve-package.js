...
module.exports = function servePackage ( req, res ) {
	...
	get( `http://registry.npmjs.org/${encodeURIComponent( qualified )}` ).then( JSON.parse )
		.then( pkg => {
			...
			return fetchBundle( pkg, tag, req.query ).then( file => {
				log.info( `[${qualified}] serving file` );
				res.sendFile( file, {
...
function fetchBundle ( pkg, version, query ) {
	...
	return sander.exists( file ).then( exists => {
		if ( exists ) {
	...
	if ( moduleEntry ) {
		...
		return rollup.rollup({
			...
			if ( bundle.imports.length > 0 ) {
				...
				return sander.writeFile( file, minify( pkg, code ) );
			}
		});
	}
...
function bundleWithBrowserify ( pkg, main, file, moduleName ) {
	log.info( `[${pkg.name}] using Browserify` );
	...
	return new Promise( ( fulfil, reject ) => {
		b.bundle( ( err, buf ) => {
			if ( err ) {
				...
				sander.writeFile( file, minify( pkg, '' + buf ) ).then( fulfil, reject );
			}
		});
	});
